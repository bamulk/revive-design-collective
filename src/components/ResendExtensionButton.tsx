...
import { resendExtensionInvoiceAction } from "@/app/(app)/stages/actions";

/**
 * Compact "Resend invoice" control for the dashboard's Outstanding
 * extensions rows — the extensions twin of ResendInvoiceButton.
 * Reuses resendExtensionInvoiceAction (generates the PDF if missing,
 * emails the client on file, stamps pdf_sent_at, which arms the
 ...
 */
export default function ResendExtensionButton({
  extensionId,
  clientEmail,
  pdfSentAt,
}: {
  ...
  const label = pdfSentAt || sent ? "Resend invoice" : "Send invoice";
  ...
  function send() {
    ...
    startTransition(async () => {
      const r = await resendExtensionInvoiceAction(extensionId);
      if (r.ok) {
        setSent(true);
        setArmed(false);
      } else {
